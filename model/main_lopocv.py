...
def evaluate_model(model, test_loader, device, batch_size, fs):
    """评估模型在test set上的指标（类似于train_utils中的val计算）"""
    model.eval()
    all_test_labels = []
    all_test_preds = []
    test_loss = 0.0
...
if __name__ == "__main__":
    for i in config['ALL_PATIENT_IDS']:
        ...
        # 创建DataLoaders
        train_dataset = TensorDataset(torch.tensor(train_split_X, dtype=torch.float32), torch.tensor(train_split_y, dtype=torch.long))
