...
class MainModel(nn.Module):
    def __init__(self, Freq=135, Time=256, num_classes=2, config=None):
        ...
        # 时频特征提取
        self.freq_branch = FrequencyBranch(in_freq=128, out_freq=64)
    ...
    def forward(self, x, return_features=False, feature_level='final'):
        ...
        freq_out = self.freq_branch(freq_raw)
        ...
        # 时间注意力池化
        x_time_avg = x_tgcn[:,-1,:,:].mean(dim=1)  # (B, 64)
        ...
        if return_features:
            if feature_level == 'freq':
                ...
                raise ValueError("feature_level must be in ['freq', 'time', 'tgcn', 'final']")
        else:
            return logits
