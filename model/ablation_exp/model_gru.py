...
class MainModel(nn.Module):
    ...
    def forward(self, x, return_features=False, feature_level='final'):
        ...
        B, T, C, feature_dim = x_feat.shape
        x_reshaped = x_feat.reshape(B * C, T, feature_dim)  # (B*22, 9, 128)

        x_tgcn, _ = self.gru(x_reshaped)  # (B*22, 9, 64)

        # 取最后时间步
        x_last = x_tgcn[:, -1, :]  # (B*22, 64)

        # 恢复通道维度
        x_last = x_last.reshape(B, C, 64)  # (B, 22, 64)

        x_pooled = x_last.mean(dim=1)  # (B, 64)

        # 分类
        logits = self.classifier(x_pooled) # (B, num_classes=2)
        ...
        if return_features:
            if feature_level == 'freq':
                ...
                return logits, x_feat.mean(dim=1).mean(dim=1)  # (B, 64)
            elif feature_level == 'tgcn':
                return logits, x_tgcn.mean(dim=1).mean(dim=1)  # (B, 64)
            elif feature_level == 'final':  # 默认
                return logits, x_pooled  # (B, 64)
            else:
                raise ValueError("feature_level must be in ['freq', 'time', 'fusion', 'tgcn', 'final']")
        else:
            return logits
