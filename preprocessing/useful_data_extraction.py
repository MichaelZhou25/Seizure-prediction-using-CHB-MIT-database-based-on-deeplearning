...
#CHB-MIT
DATASET = 'CHB-MIT'
DATA_ROOT = Path(r'D:\SeizurePrediction\CHB-MIT')
SAVE_DIR = r"D:\SeizurePrediction\EpilepsyPrediction\data_CHBMIT_1_1"
USEFUL_DATA_INFO_ROOT = Path(r'D:\SeizurePrediction\EpilepsyPrediction\datainfo\CHBMIT_useful_data_info.json')
DATA_INFO_ROOT = "CHBMIT_patient_data_statistics.csv"
PATIENT_NUM = [i for i in range(25) if i not in [0, 12, 13, 15, 24]]  # 假设有24个患者
SAMPLE_LENGTH = 5
CHANNEL_NUM = 22
...
#Siena
DATASET = 'Siena'
DATA_ROOT = Path(r'D:\SeizurePrediction\siena-scalp-eeg-database-1.0.0')
SAVE_DIR = r"D:\SeizurePrediction\EpilepsyPrediction\data_Siena_1_1"
USEFUL_DATA_INFO_ROOT = Path(r'D:\SeizurePrediction\EpilepsyPrediction\datainfo\Siena_useful_data_info.json')
DATA_INFO_ROOT = "Siena_patient_data_statistics.csv"
PATIENT_NUM = [i for i in range(18) if i not in [0, 2, 4, 8, 15]]  # 假设有24个患者
SAMPLE_LENGTH = 5
CHANNEL_NUM = 19
SAMPLING_OVERLAP = 0.5
TARGET_CHANNELS = ['EEG F3', 'EEG T3', 'EEG T5', 'EEG F7', 'EEG Pz', 'EEG F8',
                   'EEG O2', 'EEG Fc1', 'EEG C4', 'EEG P3', 'EEG Cz', 'EEG CZ',  #bug
                   'EEG F4', 'EEG C3', 'EEG T6', 'EEG O1',
                   'EEG Fp1', 'EEG T4', 'EEG P4', 'EEG Fz']
'''
def read_edf_segment(
    ...
    with pyedflib.EdfReader(str(file_path)) as f:
        ...
        excluded_channels = [ch for ch in target_channels if ch not in common_channels]
        ...
        if DATASET == 'Siena':
            # 降采样倍数q=512/256=2，使用FIR滤波器抗混叠
            data = decimate(
                x=data,  # 输入信号（shape: [channels, timepoints]）
                q=2,  # 降采样倍数（整数）
                ftype='fir',  # 滤波器类型（FIR线性相位，适合EEG）
                axis=1,  # 时间轴（第1维，对应timepoints）
                zero_phase=True  # 零相位滤波（避免相位偏移，可选但推荐）
            )
            sample_rate = 256  # 更新采样率
...
if __name__ == "__main__":
    ...
    for i, patient_data in enumerate(useful_data_info):
        ...
        with h5py.File(save_path_interictal, 'w') as f:
            ...
            for seg_idx, seg in enumerate(all_interictal_data, 1):
                ...
                if total_interictal_sample >= 3 * total_preictal_sample:
    ...
    # 保存为 CSV 文件
    csv_save_path = os.path.join(SAVE_DIR, DATA_INFO_ROOT)
